...
import { signInWithEmailAndPassword, signInWithPopup } from "firebase/auth"; // Importer la méthode Firebase
import { useState } from "react";
import toast from "react-hot-toast";
import { MdOutlineMail, MdPassword } from "react-icons/md";
import { Link } from "react-router-dom";
import googleIcon from "../../../components/svgs/googleIcon.png"; // Chemin vers l'icône Google
import { authKey } from "../../../config/cometchatConfig";
...
const LoginPage = () => {
	...
	const authKey = "2f1b702468f4058be0f839eb69c9cd3422432c41"; // Remplacez par votre authKey CometChat
	...
	const createUserInCometChat = async (user) => {
		...
		try {
		  ...
		  console.log("Utilisateur créé dans CometChat :", newUser);
		} catch (error) {
	  ...
	  const loginToCometChat = async (uid) => {
		try {
		  ...
		  toast.success("Connecté à CometChat !");
		} catch (error) {
	...
	const {
		...
		mutationFn: async ({ email, firebaseIdToken }) => {
			const res = await fetch("/api/auth/login", {
				...
				body: JSON.stringify({ email, firebaseIdToken }), // Envoi de l'email et du token Firebase
			});
			...
			if (!res.ok) {
				...
				const { error } = data;
				if (error === "Veuillez fournir un email et un mot de passe.") {
	...
	const handleGoogleLogin = async () => {
		...
		try {
		  ...
		  // Si tout est réussi, effectuez une mutation pour gérer l'utilisateur
